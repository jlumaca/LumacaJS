...
document.addEventListener("DOMContentLoaded",async () => {
        ...
        try {
            ...
            if (window.location.pathname.includes("autos.html")) {
                if (!usuario) {
                    mostrarMensaje(
                        "login_msg",
...
document.getElementById("tabla_autos")?.addEventListener("click",function (e) {if (e.target.classList.contains("btn-eliminar")) {eliminarAuto(e.target.dataset.codigo);}});
...
document.getElementById("busqueda")?.addEventListener("input",function () {
            ...
            let filtrados = autos.filter(a => a.marca.toLowerCase().includes(texto) || a.modelo.toLowerCase().includes(texto));
            mostrarAutos(filtrados);
        }
    );

document.getElementById("codigo")?.addEventListener("keydown",function (e) {
    if (e.key === "Enter") { buscarAuto(); }
    }
);
